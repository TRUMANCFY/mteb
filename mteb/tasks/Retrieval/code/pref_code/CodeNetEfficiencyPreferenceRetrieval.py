...
class CodeNetEfficiencyPreferenceRetrieval(MultilingualTask, AbsTaskRetrieval):
    ...
    metadata = TaskMetadata(
        name="CodeNetEfficiencyPreferenceRetrieval",
        description="The dataset is derived from CodeNet to benchmark retrievers' performance on efficiency awareness.",
    ...
    def load_data(self, **kwargs):
        ...
        for _line in tqdm(qrels_effi_lines):
            qid = _line['qid']
            ...
            for pos_doc_id in pos_doc_ids:
                ...
                if qid not in self.queries[_lang][self._EVAL_SPLIT]:
                    self.queries[_lang][self._EVAL_SPLIT][qid] = str(query_effi_dict[qid]['text'])
